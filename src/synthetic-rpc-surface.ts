...
export function buildSyntheticRpcRuntime(
  ...
  const rewriteRuntimeExpression =
    options.rewriteRuntimeExpression ?? ((expression) => expression);
  const lines = [
    ...(definition.runtimePreludeLines ?? []),
    ...getBindings(definition).map((binding) => {
      const expression = rewriteRuntimeExpression(
        binding.runtimeExpression,
        binding,
      );
