...
export type RpcMethodCallerFromCallable<
  TCallable extends (...args: any[]) => Promise<any>,
> = (
  method: RpcMethodRefFromCallable<TCallable>,
  ...args: Parameters<TCallable>
  ...
  const build = (parts: string[]): unknown => {
    ...
    const proxy = new Proxy(target, {
      ...
      apply(target, _thisArg, argArray) {
        ...
        if (!caller) {
          throw new Error(
            `RPC reference ${cacheKey} cannot be invoked directly. Bind a caller or resolve it through your RPC caller.`,
          );
        }
        return caller(
          target as RpcMethodRef<any[], any>,
          ...(argArray as any[]),
        );
      },
    });
