...
- `@nogg-aholic/nrpc-cli` is a development-time dependency used to generate artifacts
- `@nogg-aholic/nrpc` is the runtime dependency used by the server and typed runtime clients
...
Generation code should live under `scripts/`.
Runtime code should live under `src/`.

This keeps the package boundary clear and avoids pulling generator concerns into runtime sources.

## Runtime Endpoints

`@nogg-aholic/nrpc` is the package that owns the runtime side of:

- `/rpc` via `createRpcFetchRequestHandler`
- `/api/...` and `/api/... .nrpc` via `createSyntheticHttpRouteHandler`
- `/docs` via generated docs runtime artifacts
- `/mcp` via `createMcpHttpHandler`

Those runtime endpoints are set up from generated artifacts, but they are not generated by hand and should not depend on `@nogg-aholic/nrpc-cli` at runtime.

For a step-by-step runtime wiring guide, see [INSTRUCTION_MANUAL.md](INSTRUCTION_MANUAL.md).
...
2. generate one endpoint surface from that root with `@nogg-aholic/nrpc-cli`
3. let `nRPC` emit all method codecs and a registry automatically

The normal generated runtime outputs are consumed from your app's `src/generated/` folder.
The runtime package should consume those generated files, not regenerate them.
